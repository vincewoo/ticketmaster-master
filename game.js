...
let gameState = {
    ...
    // Blackjack CAPTCHA state
    blackjackDeck: [],
    blackjackPlayerHand: [],
    blackjackDealerHand: [],
    blackjackPlayerScore: 0,
    blackjackDealerScore: 0,
    isBlackjackPlayerTurn: false,
};
...
function initiateCheckout() {
    ...
    if (hasCompetition || Math.random() < 0.6) {
        ...
        if (captchaType < 0.16) {
            showCaptcha();
        } else if (captchaType < 0.32) {
            showGasPumpCaptcha();
        } else if (captchaType < 0.48) {
            showPuzzleCaptcha();
        } else if (captchaType < 0.64) {
            showFishingCaptcha();
        } else if (captchaType < 0.8) {
            showNBACaptcha();
        }
        else {
            showBlackjackCaptcha();
        }
    } else {
        completeCheckout();
    }
...
function showPuzzleCaptcha() {
    ...
    showModal('puzzle-captcha-modal');
    ...
    gameState.captchaInterval = setInterval(() => {
        ...
        if (gameState.captchaTimeRemaining <= 0) {
            ...
            setTimeout(() => {
                hideModal('puzzle-captcha-modal');
            }, 1500);
        }
    }, 1000);
}
...
function generatePuzzle() {
    ...
    // Draw the hole background with shadow
...
function drawJigsawPiece(ctx, x, y, size) {
...
function updatePuzzlePiecePosition() {
...
function showPuzzleCaptchaError(message) {
    const errorEl = document.getElementById('puzzle-captcha-error');
    errorEl.textContent = message;
    errorEl.classList.remove('hidden');
}

// Cancel puzzle CAPTCHA
function cancelPuzzleCaptcha() {
    clearInterval(gameState.captchaInterval);
    hideModal('puzzle-captcha-modal');
}
...
function dealerBlackjackTurn() {
    if (gameState.blackjackDealerScore < 17) {
        gameState.blackjackDealerHand.push(gameState.blackjackDeck.pop());
        renderBlackjackHands();
        updateBlackjackScores();
...
function cancelBlackjackCaptcha() {
    hideModal('blackjack-captcha-modal');
}
...
function setupEventListeners() {
    ...
    document.getElementById('puzzle-submit').addEventListener('click', verifyPuzzle);
    ...
    document.getElementById('blackjack-hit-btn').addEventListener('click', handleBlackjackHit);
    document.getElementById('blackjack-stand-btn').addEventListener('click', handleBlackjackStand);
    document.getElementById('blackjack-cancel-btn').addEventListener('click', cancelBlackjackCaptcha);
...
function setupDebugPanel() {
    ...
    document.getElementById('debug-blackjack-captcha').addEventListener('click', () => {
        if (gameState.isRunning) {
            showBlackjackCaptcha();
        }
    });
